...
const EditorRichTextEditor = ({ content, onChange, onSave, isSaving }) => {
    ...
    const editor = useEditor({
        ...
        content: content || '<p>Start from here...</p>',
        onUpdate: ({ editor }) => {
                ...
                <div className="bg-white border-b border-gray-200 p-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    {onSave && (
                        <button
                            onClick={onSave}
                            disabled={isSaving}
                            className={`px-4 py-2 rounded text-white flex items-center space-x-2 transition-colors ${isSaving ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
                                }`}
                            aria-label="Save"
                        >
                            <Save size={16} />
